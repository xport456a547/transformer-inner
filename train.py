    ...
    def train(self, train_cfg):
        ...
        for epoch in range(train_cfg.n_epochs):
            ...
            for i, batch in enumerate(iter_bar):
                ...
                iter_bar.set_description('[Train] Iter %d/%d (loss=%5.3f accuracy=%5.3f bpc=%5.3f)' % (
                    epoch + 1, train_cfg.n_epochs, loss.item() * train_cfg.accumulation_steps, acc,
                    loss.item() * train_cfg.accumulation_steps / math.log(2)))
            ...
            with torch.no_grad():
                ...
                for i, batch in enumerate(iter_bar):
                    ...
                    iter_bar.set_description('[Eval] Iter %d/%d (loss=%5.3f accuracy=%5.3f bpc=%5.3f)' % (
                        epoch + 1, train_cfg.n_epochs, loss.item(), acc, loss.item() / math.log(2)))
    ...
    def train(self, train_cfg):
        ...
        for epoch in range(train_cfg.n_epochs):
            ...
            for i, batch in enumerate(iter_bar):
                ...
                iter_bar.set_description('Iter %d (loss=%5.3f accuracy=%5.3f)' % (
                epoch + 1, loss.item() * train_cfg.accumulation_steps, acc))
